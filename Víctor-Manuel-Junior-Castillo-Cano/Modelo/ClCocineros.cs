...
namespace Modelo
{
    public class ClCocineros
    {
        ...
        public static int ModificarCocineros(int idCocinero, string documento, string correo, string nombres,
           string direccion, string telefono)
        {
            int resultado = 0;

            EntidadDataContext entity = new EntidadDataContext();
            cocineros persona = Buscar_IdCocinero(idCocinero);
        ...
        public static cocineros Buscar_IdCocinero(int id)
        {
            EntidadDataContext context = new EntidadDataContext();
            cocineros persona = null;

            persona = context.cocineros.Where(p => p.idCocineros == id).Select(p => p).First();
